const CACHE_NAME = 'pickleball-coach-ai-v73-taalpopup-origineel';

const STABLE_STYLE = `<style id="pickleball-stable-style-v73">
...
const STABLE_SCRIPT = `<script id="pickleball-stable-script-v73">
(function(){
  function hideDebug(){
  ...
  document.addEventListener('click',function(e){
    ...
    if(action.indexOf('setTaal(')>-1){setTimeout(function(){fixLayout();applyOriginalLanguage();},300);return;}
...
function injectStableHead(html){
  let next=html;
  if(!next.includes('pickleball-stable-style-v73')) next=next.replace('</head>',STABLE_STYLE+'\n</head>');
  if(!next.includes('pickleball-stable-script-v73')) next=next.replace('</body>',STABLE_SCRIPT+'\n</body>');
  return next;
}
