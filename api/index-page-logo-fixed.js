...
function injectPageAdditions(html) {
  let next = html;

  if (!next.includes('large-accordion-chevrons-direct')) {
    next = next.replace('</head>', `${CHEVRON_STYLE}\n</head>`);
  }

  if (!next.includes('title-logo-mark-style')) {
    next = next.replace('</head>', `${HEADER_LOGO_STYLE}\n</head>`);
  }

  if (!next.includes('<h1 class="brand-title">')) {
    next = next.replace(/<h1[^>]*>\s*Pickleball Coach\s*<\/h1>/, TITLE_WITH_LOGO);
  }
...
export default async function handler(req, res) {
  ...
  const html = injectPageAdditions(await response.text());
