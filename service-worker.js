const CACHE_NAME = 'pickleball-coach-ai-v70-originele-vertaling';

const STABLE_STYLE = `<style id="pickleball-stable-style-v70">
...
const STABLE_SCRIPT = `<script id="pickleball-stable-script-v70">
(function(){
  function hideDebug(){
...
function injectStableHead(html){
  let next=html;
  if(!next.includes('pickleball-stable-style-v70')) next=next.replace('</head>',STABLE_STYLE+'\n</head>');
  if(!next.includes('pickleball-stable-script-v70')) next=next.replace('</body>',STABLE_SCRIPT+'\n</body>');
  return next;
}
