const CACHE_NAME = 'pickleball-coach-ai-v75-settaal-hard-fix';

const STABLE_STYLE = `<style id="pickleball-stable-style-v75">
...
const STABLE_SCRIPT = `<script id="pickleball-stable-script-v75">
(function(){
  ...
  hideDebug();fixLayout();applyOriginalLanguage();document.addEventListener('DOMContentLoaded',function(){hideDebug();fixLayout();applyOriginalLanguage();});window.addEventListener('load',function(){hideDebug();fixLayout();applyOriginalLanguage();setTimeout(function(){fixLayout();applyOriginalLanguage();},500);setTimeout(function(){fixLayout();applyOriginalLanguage();},1600);});setInterval(function(){hideDebug();fixLayout();},3000);
})();
</script>`;

function injectStableHead(html){let next=html;if(!next.includes('pickleball-stable-style-v75'))next=next.replace('</head>',STABLE_STYLE+'\n</head>');if(!next.includes('pickleball-stable-script-v75'))next=next.replace('</body>',STABLE_SCRIPT+'\n</body>');return next;}
